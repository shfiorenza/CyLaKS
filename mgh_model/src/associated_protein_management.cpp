...
//				std::cout << bound_I << std::endl;
				...
				if(parameters_->microtubules.count > 1){
					for(int x(0); x <= dist_cutoff_; x++){
						std::string bound_II_same = "bound_II_same_"
							+ std::to_string(x) + "_"
							+ std::to_string(x_dub) + "_"
							+ std::to_string(n_neighbs);
						std::string bound_II_oppo = "bound_II_oppo_"
							+ std::to_string(x) + "_"
							+ std::to_string(x_dub) + "_"
							+ std::to_string(n_neighbs);
						events_.emplace_back(this, ID++, -40, 
								"diff_II_to", bound_II_same, 
								p_diffuse_ii_to_both_[n_neighbs][x_dub][x],
								&n_bound_ii_teth_same_[n_neighbs][x_dub][x],
								&bound_ii_teth_same_[n_neighbs][x_dub][x], 
								ran_int, binomial);
						events_.emplace_back(this, ID++, -41, 
								"diff_II_fr", bound_II_same, 
								p_diffuse_ii_fr_both_[n_neighbs][x_dub][x],
								&n_bound_ii_teth_same_[n_neighbs][x_dub][x],
								&bound_ii_teth_same_[n_neighbs][x_dub][x], 
								ran_int, binomial);
						events_.emplace_back(this, ID++, -50, 
								"diff_II_to_fr", bound_II_oppo, 
						...
						events_.emplace_back(this, ID++, -50, 
								...
								ran_int, binomial);
						events_.emplace_back(this, ID++, 41, 
								"unbind_II_to", bound_II_same, 
								p_unbind_ii_to_teth_[n_neighbs][x_dub][x], 
								&n_bound_ii_teth_same_[n_neighbs][x_dub][x],
								&bound_ii_teth_same_[n_neighbs][x_dub][x], 
								ran_int, binomial);
						events_.emplace_back(this, ID++, 42, 
								"unbind_II_fr", bound_II_oppo,
								p_unbind_ii_fr_teth_[n_neighbs][x_dub][x], 
								&n_bound_ii_teth_oppo_[n_neighbs][x_dub][x],
								&bound_ii_teth_oppo_[n_neighbs][x_dub][x], 
								ran_int, binomial);
		...
		events_.emplace_back(this, ID++, 50, "tether_free", "unteth_mots", 
				p_tether_free_, &properties_->kinesin4.n_bound_untethered_, 
				&properties_->kinesin4.bound_untethered_, 
				ran_int, binomial);
		events_.emplace_back(this, ID++, 60, "untether_free", "free_teth", 
				p_untether_free_, &n_free_teth_, &free_teth_,
				ran_int, binomial);
	}
...
//			std::cout << target_pop;
//			printf(" will be corrected in secondary scan only; root is ");
...
//		printf("\n");
		...
		for(int i_root(0); i_root < n_roots; i_root++){
			...
			else target_root = std::string("nope");
	...
	for(int i_root(0); i_root < n_roots; i_root++){
		...
		*/
		...
		for(int i_entry(0); i_entry < n_sec_entries[i_root]; i_entry++){
			...
			if(coupled) IDs_by_root_[i_root][i_entry] = -1 * entry_ID;
			else IDs_by_root_[i_root][i_entry] = entry_ID;
//			printf(" %i,", IDs_by_root_[i_root][i_entry]);
		}
//		printf("\n");
	}
}
...
double AssociatedProteinManagement::GetWeight_Bind_II_Teth(){
	...
	for(int x_dub(2*comp_cutoff_); x_dub <= 2*teth_cutoff_; x_dub++){
		...
		for(int i_xlink(0); i_xlink < n_bound; i_xlink++){
			AssociatedProtein *xlink = std::get<POP_T*>(bound_i_teth_
	...
	&& !(site->index_ == site->mt_->n_sites_ - 1 && dx == 1)){
		...
		if(!new_site->occupied_){
			...
			&& !(new_site->index_ == site->mt_->n_sites_ - 1 && dx == 1)){
				Tubulin *new_neighb 
					= &site->mt_->lattice_[new_site->index_ + dx];
