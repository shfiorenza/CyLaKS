...
for i_seed = 1 : n_seeds
    ...
    mt_data = fread(mt_data_file, n_mts * n_datapoints, '*double');
    fclose(mt_data_file);
...
for i_mt = 1 : n_mts
    ...
    hold on
    errorbar(taus, MSD(i_mt, :), MSD_err(i_mt, :), '.', 'LineWidth', 2, 'MarkerSize', 14);
    plot([0 max_tau], [0 2*D_expected(i_mt)*max_tau], '--', 'LineWidth', 1.5); %, ...
    ax = gca;
    ax.FontSize = 10;   
    xlabel("Tau (s)", 'FontSize', 14);
    ylabel("Mean squared displacement (\mum^2)", 'FontSize', 14);
    title({sprintf("L = %g", mt_lengths(i_mt) * site_size) + " \mum", " "}, 'FontSize', 12);
...
ylim([1 2]);
axis off
legend(["Simulation", "Theory"], 'location', 'northwestoutside', 'FontSize', 14);
sgtitle("Microtubule diffusion with no external forces", 'FontSize', 16);