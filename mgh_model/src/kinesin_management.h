...
private:
  ...
  struct event {
    event(int i, int code, std::string l, std::string t_p,
          std::function<int(double, int)> p_dist, int *pop, double p)
        : index_(i), kmc_code_(code), label_(l), target_pop_(t_p),
          prob_dist_(p_dist), pop_ptr_(pop), p_occur_(p) {}
...
public:
  ...
  // Event probabilities
  double p_bind_i_{0.0};
  double p_bind_i_tethered_{0.0};
  double p_bind_ATP_{0.0};
  double p_hydrolyze_{0.0};
  double p_hydrolyze_stalled_{0.0};
  double p_bind_ii_{0.0};
  double p_unbind_ii_{0.0};
  double p_unbind_i_{0.0};
  double p_unbind_i_stalled_{0.0};
  double p_tether_free_{0.0};
  double p_tether_bound_{0.0};
  double p_untether_free_{0.0};
