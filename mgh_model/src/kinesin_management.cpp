...
void KinesinManagement::InitializeEvents() {
  ...
  int index = 0;
  events_.emplace_back(event(index++, 10, "bind_i", "unoccupied", binomial,
                             &properties_->microtubules.n_unoccupied_,
                             p_bind_i_));
  events_.emplace_back(event(index++, 20, "bind_ATP", "bound_NULL", binomial,
                             &n_bound_NULL_, p_bind_ATP_));
  events_.emplace_back(event(index++, 30, "hydrolyze", "bound_ATP", binomial,
                             &n_bound_ATP_, p_hydrolyze_));
  events_.emplace_back(event(index++, 31, "hydrolyze_stalled",
                             "bound_ATP_stalled", binomial,
                             &n_bound_ATP_stalled_, p_hydrolyze_stalled_));
  events_.emplace_back(event(index++, 40, "bind_ii", "bound_ADPP_i", binomial,
                             &n_docked_, p_bind_ii_));
  events_.emplace_back(event(index++, 50, "unbind_ii", "bound_ADPP_ii",
                             binomial, &n_bound_ADPP_ii_, p_unbind_ii_));
  events_.emplace_back(event(index++, 60, "unbind_i", "bound_ADPP_i", binomial,
  ...
  if (parameters_->motors.tethers_active) {
    ...
    for (int x_dub(2 * comp_cutoff_); x_dub <= 2 * dist_cutoff_; x_dub++) {
      events_.emplace_back(event(
          index++, 400 + x_dub, "bind_ii_teth", "bound_ADPP_i_teth", binomial,
          &n_docked_tethered_[x_dub], p_bind_ii_tethered_[x_dub]));
    }
    for (int x_dub(2 * comp_cutoff_); x_dub <= 2 * dist_cutoff_; x_dub++) {
      events_.emplace_back(event(
          index++, 600 + x_dub, "unbind_i_teth", "bound_ADPP_i_teth", binomial,
          &n_bound_ADPP_i_tethered_[x_dub], p_unbind_i_tethered_[x_dub]));
    }
    for (int x_dub(2 * comp_cutoff_); x_dub <= 2 * dist_cutoff_; x_dub++) {
    ...
    events_.emplace_back(event(index++, 70, "tether_free", "untethered_xlinks",
                               binomial, &properties_->prc1.n_bound_unteth_,
                               p_tether_free_));
    ...
    events_.emplace_back(event(index++, 71, "tether_bound", "untethered_xlinks",
                               poisson_teth, &n_bound_untethered_,
                               p_tether_bound_));
    events_.emplace_back(event(index++, 80, "untether_free", "free_tethered",
                               binomial, &n_free_tethered_, p_untether_free_));
    for (int x_dub(2 * comp_cutoff_); x_dub <= 2 * dist_cutoff_; x_dub++) {
      events_.emplace_back(
          event(index++, 800 + x_dub, "untether_bound", "bound_tethered",
                binomial, &n_bound_tethered_[x_dub], p_untether_bound_[x_dub]));
    }
  }
}
...
void KinesinManagement::UpdateBoundADPP_I() {
  ...
  for (int i_motor = 0; i_motor < n_active_; i_motor++) {
    Kinesin *motor = active_[i_motor];
    if (motor->heads_active_ == 1 && !motor->IsStalled()) {
  ...
  for (int i_motor = 0; i_motor < n_active_; i_motor++) {
    ...
    if (motor->heads_active_ == 1 && motor->IsStalled()) {
      ...
      if (!motor->tethered_) {
        if (motor->head_one_.site_ != nullptr &&
            motor->head_one_.ligand_ == "ADPP") {
          bound_ADPP_i_stalled_[n_bound_ADPP_i_stalled_] = &motor->head_one_;
          n_bound_ADPP_i_stalled_++;
        }
        if (motor->head_two_.site_ != nullptr &&
            motor->head_two_.ligand_ == "ADPP") {
          bound_ADPP_i_stalled_[n_bound_ADPP_i_stalled_] = &motor->head_two_;
          n_bound_ADPP_i_stalled_++;
        }
      }
      // Motors with satellite xlinks behave as if untethered
      ...
      for (int i_step = 0; i_step < 2; i_step++) {
        ...
        if (index == 0) {
          ...
          while (events_[i_bind_ii].n_expected_ +
            ...
            double p_mid =
                events_[i_bind_ii].p_occur_ + events_[i_unbind_i_teth].p_occur_;
    ...
    && events_[i_entry].n_expected_ > 0){
            ...
            for(int i = 0; i < n_bound_NULL_; i++){
                    printf("motor ID is %i - %s\n",
                                    ...
                                    bound_NULL_[i]->trailing_ ? "trailing" :
    "leading");
            }
    }
    */
...
void KinesinManagement::KMC_Bind_ATP() {
  ...
  if (n_bound_NULL_ > 0) {
    ...
    if (!head->trailing_) {
      ...
      else if ((i_site == 0 && dx == -1) && (i_site == mt_length && dx == 1))
        head->motor_->ChangeConformation();
...
void KinesinManagement::KMC_Hydrolyze() {
  ...
  if (n_bound_ATP_ > 0) {
    ...
    // Verify correctness
    if (head->ligand_ != "ATP" || head->site_ == nullptr) {
