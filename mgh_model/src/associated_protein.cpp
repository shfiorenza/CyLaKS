...
void AssociatedProtein::InitializeLookupTables() {
  ...
  for (int n_neighbs(0); n_neighbs <= max_neighbs_; n_neighbs++) {
    double weight_neighb = neighb_weights[n_neighbs];
...
void AssociatedProtein::ForceUnbind() {
  ...
  //  printf("unbound stage-2 xlink - FORCED\n");
  ...
  if (tethered_ == false) {
    ...
    double p_unbind_to =
        properties_->prc1.p_unbind_ii_to_teth_[neighbs_to][x_dub][x_dist_];
    double p_unbind_from =
        properties_->prc1.p_unbind_ii_fr_teth_[neighbs_fr][x_dub][x_dist_];
