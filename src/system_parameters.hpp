...
struct SysParams {
  ...
  struct FilamentParams {
    ...
    Vec<double> applied_force;  // Perpetually applied force on each MT (pN)
    Vec<bool> dim_enabled;      // x- & y-axis movement toggle
    double y_dist_init;         // Vertical distance between each MT
  ...
  struct XlinkParameters {
    ...
    double d_i;        // Diffusion coefficient; um^2/s
    double d_ii;       // Diffusion coefficient; um^2/s
