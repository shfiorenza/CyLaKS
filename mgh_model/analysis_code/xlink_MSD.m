clear all;
i_mts_scanned = [0, 1];
n_mts_scanned = length(i_mts_scanned);
...
for i_mt = i_mts_scanned + 1
    ...
    for i_offset = 1 : n_offsets
        ...
        D = fit(2) / 2
        subplot(n_mts_scanned, n_offsets, i_offset + n_offsets * (i_mt - 1))
        hold all
        % Plot data
        errorbar(taus, MSD, MSD_err, '.', 'MarkerSize', 14, 'LineWidth', 2);
        % Plot fit
       % plot(taus, taus_padded * fit, '--', 'LineWidth',  1.5);
        ylabel('MSD (\mum^2)', 'FontSize', 12);
        xlabel('Tau (s)', 'FontSize', 14);
        title( mt_index + "\_" + offset, 'FontSize', 14);
        %title("Singly-bound crosslinkers", 'FontSize', 16);
        %legend('Data', 'Fit', 'location', 'best', 'FontSize', 12);
