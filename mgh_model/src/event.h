...
private:
  ENTRY_T GetActiveEntry() {
    ...
    if (*n_avail_ > 0)
      ...
      target = manager_->CheckScratchFor(target_pop_);
    return target;
  };

public:
  Event(){};
  Event(MGMT_T manager, int ID, int code, std::string name, std::string target,
        double p_occur, int *avail, std::vector<ENTRY_T> *pool,
