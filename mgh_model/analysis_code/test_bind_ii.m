...
speciesID = 1;
static_head_pos = dist_cutoff + mt_coords(1);
...
for i_data = 1 : n_datapoints
   ...
   if isempty(dynamic_head_pos)
       ...
       dynamic_head_pos = dynamic_head_pos + mt_coords(2);
       % Get x_dist for this configuration (-dist_cutoff to +dist_cutoff)
       x_dist = abs(dynamic_head_pos - static_head_pos);
       % Convert to an index between 1 and 2*dist_cutoff + 1
       if x_dist > round(x_dist)
           index = round(x_dist) + 1;
       else
           index = round(x_dist) + dist_cutoff + 1;
       end
...
% Need to re-arrange avg_occu data so that it's symmetric
% I.e., need to flip first half of data set along vertical axis
avg_occu_sym = zeros(2* dist_cutoff + 1, 1);
for x = 1 : dist_cutoff + 1
    avg_occu_sym(x) = avg_occu(dist_cutoff + 2 - x);
end
for x = dist_cutoff + 1 : 2 * dist_cutoff + 1
   avg_occu_sym(x) = avg_occu(x);
end

% Plot avg_occu data
plot(linspace(-dist_cutoff, dist_cutoff, 2*dist_cutoff + 1), ... 
    avg_occu_sym, '*', 'MarkerSize', 14, 'LineWidth', 3);
hold on
...
function u = delta_u(x)
    ...
    r_x = (x - offset) * site_size;
    dr = sqrt(32^2 + r_x^2) - 32;
